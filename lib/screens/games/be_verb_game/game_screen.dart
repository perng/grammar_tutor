...
class BeVerbGameScreen extends StatefulWidget {
  final int levelIndex;
  final String routePrefix;

  const BeVerbGameScreen({
    super.key,
    required this.levelIndex,
    this.routePrefix = '/be-verb-game',
  });

  @override
  State<BeVerbGameScreen> createState() => _BeVerbGameScreenState();
}

class _BeVerbGameScreenState extends State<BeVerbGameScreen> {
  List<Word> _words = [];
    ...
    for (final match in matches) {
      ...
      if (part.startsWith('[') && part.endsWith(']')) {
        ...
        if (forms.isNotEmpty) {
          ...
          // If separator is '|', first one is correct. If '-', all are correct
  ...
  Future<void> _checkAnswers() async {
    ...
    if (mounted) {
      await Provider.of<ProgressProvider>(
        ...
        'be_verb_adjectives-${widget.levelIndex}',
        percentage,
      );
    }
    if (percentage == 100) {
